...
from ChemicalDice.descriptors.quantum_need import *
...
def get_mopac_prerequisites():
    print("Skipping Mopac setup (manual/offline mode)")

def setup_mopac_and_morse():
    base_dir = os.path.dirname(__file__)
    resource_dir = os.path.join(base_dir, "resources")

    mopac_tar = os.path.join(resource_dir, "mopac-22.1.1-linux.tar.gz")
    morse_cpp = os.path.join(resource_dir, "3dmorse.cpp")
    tchar_h = os.path.join(resource_dir, "tchar.h")
    mingw_h = os.path.join(resource_dir, "_mingw.h")

    # ---------------- MOPAC ----------------
    if not os.path.exists("mopac-22.1.1-linux"):
        if not os.path.exists(mopac_tar):
            raise RuntimeError("Missing mopac tar file in resources/")
        
        subprocess.run(["tar", "-xzf", mopac_tar], check=True)
        print("Mopac extracted")

    # ---------------- MORSE ----------------
    if os.name == "nt":
        if not os.path.exists("3dmorse.exe"):
            raise RuntimeError(
                "Windows requires 3dmorse.exe. Download manually:\n"
                "https://github.com/devinyak/3dmorse"
            )
        print("3dmorse.exe found")

    else:
        if not all(os.path.exists(f) for f in [morse_cpp, tchar_h, mingw_h]):
            raise RuntimeError(
                "Missing Morse source files.\n"
                "Place the following in resources/:\n"
                " - 3dmorse.cpp\n - tchar.h\n - _mingw.h"
            )

        # Copy locally (avoid editing originals)
        subprocess.run(["cp", morse_cpp, "."], check=True)
        subprocess.run(["cp", tchar_h, "."], check=True)
        subprocess.run(["cp", mingw_h, "."], check=True)

        # Patch includes
        with fileinput.FileInput("3dmorse.cpp", inplace=True) as file:
            for line in file:
        ...
        with fileinput.FileInput("tchar.h", inplace=True) as file:
            for line in file:
                print(line.replace('<_mingw.h>', '"_mingw.h"'), end='')

        # Compile
        subprocess.run(['g++', '3dmorse.cpp', '-o', '3dmorse'], check=True)

        # Cleanup
        for f in ["3dmorse.cpp", "tchar.h", "_mingw.h"]:
            if os.path.exists(f):
                os.remove(f)

        print("Morse compiled successfully")
