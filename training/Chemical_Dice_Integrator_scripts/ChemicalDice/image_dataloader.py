...
def load_filenames_and_labels_multitask2(txt_file):
    #assert task_type in ["classification", "regression"]
    df = pd.read_csv(txt_file)
...
def get_datasets2(dataset, dataroot, data_type="raw"):
    #assert data_type in ["raw", "processed"]

    image_folder = os.path.join(dataroot, "224")
    txt_file = os.path.join(dataset)
